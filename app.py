    ...
    for d in data:
        ...
        if due_day_raw in ("", None, "null"):
            ...
            try:
                ...
                due_day = None
        paid_off = 1 if d.get("paid") else 0
        paid_this_month = 1 if d.get("paid_this_month") else 0
        conn.execute('''
            INSERT INTO debts (name, balance, rate, payment, due_day, paid, credit_limit, paid_this_month)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (name, balance, rate, payment, due_day, paid_off, credit_limit, paid_this_month))
    conn.commit()
    conn.close()
